...
버전: **v1.5.0** (이전 버전은 `../dist/` 에 zip 으로 보관)
...
| **v1.5.0** | **인쇄 로딩 오버레이**(CSS 스피너) + **편집기 대각선 리사이즈 핸들** + **전용 프로필 인쇄 .bat**(평소 크롬과 분리 → kiosk-printing 항상 적용). |
...
### 크롬 인쇄창까지 없애기 (완전 자동 인쇄) — 전용 프로필 방식

크롬은 보안상 확장만으로 인쇄창을 못 없앤다. `--kiosk-printing` 플래그가 필요한데,
이 플래그는 **그 플래그로 시작된 크롬 프로세스에만** 적용된다. 평소 크롬이 떠 있으면
같은 프로세스에 합쳐져 **플래그가 무시**된다(그래서 기존 방식은 인쇄창이 계속 떴음).

→ **해결: 전용 프로필로 띄우는 `유비샵-인쇄모드.bat`**. 평소 크롬과 분리된 별도 크롬이라
플래그가 항상 적용된다. 확장도 자동 로드(`--load-extension`)된다.

1. 동봉 **`유비샵-인쇄모드.bat`** 더블클릭 → 전용 인쇄 크롬이 유비샵 목록으로 열림.
2. **이 창에서 그대로 작업**하면 `바코드인쇄` 시 인쇄창 없이 즉시 인쇄(+ 로딩 애니메이션).
   - ※ **매 인쇄마다 .bat 을 실행할 필요 없음.** 이 창을 띄워두고 계속 인쇄하면 됨.
     (아침에 한 번 더블클릭 → 종일 이 창에서 인쇄)
3. 최초 1회: 이 전용 창에서 **유비샵 로그인**, Windows **기본 프린터 = Zebra GX430t**
   (용지 60×10mm), 처음 뜨는 **위치 편집기에서 위치 맞추고 저장**.

> 평소 바탕화면 크롬 아이콘으로 열면 인쇄창이 뜬다. **반드시 이 .bat 으로** 유비샵을 열 것.
> 미리보기를 보고 인쇄하려면 `config.js` 의 `print.previewBeforePrint: true`.
