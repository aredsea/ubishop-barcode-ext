...
버전: **v1.6.0** (이전 버전은 `../dist/` 에 zip 으로 보관)
...
| **v1.6.0** | **위치 저장을 localStorage 단일 소스로 단순화**(재설치 없이 새로고침만으로 인쇄 반영) + 저장 확인 배너 + 자동인쇄 설정을 **VBScript**로 재구현(한글깨짐/실행정책 해결) + 옵션페이지는 안내용으로. |
