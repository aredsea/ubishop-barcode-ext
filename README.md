...
버전: **v1.7.0** (이전 버전은 `../dist/` 에 zip 으로 보관)
저장소: https://github.com/aredsea/ubishop-barcode-ext

> ✅ **v1.6.0 위치 저장 방식 단순화(재설치 불필요)**: 위치 저장을 `chrome.storage`+bridge
> 에서 **localStorage(유비샵 origin) 단일 소스**로 바꿨다. 인쇄 페이지에서 동기적으로
> 읽고 써서 **저장한 위치가 항상 인쇄에 반영**된다. label.js 는 원격 파일이라
> **페이지 새로고침만으로 자동 적용**(인터넷 연결 시). 재설치/새로고침 버튼 불필요.
>
> ※ 위치 설정은 **반드시 유비샵 페이지 우하단 "🏷 라벨위치" 버튼**(또는 최초실행 편집기)에서.
> 크롬 확장 옵션 페이지에서는 origin 이 달라 적용되지 않는다(그쪽은 안내만 표시).
...
├── 자동인쇄-설정.bat / autoprint-setup.vbs  # ★ 자동인쇄 1회 설정(시작프로그램+바로가기+백그라운드OFF)
├── options.html           # 확장 옵션(안내만 — 위치설정은 페이지 내 버튼에서)
├── src/
...
| v1.5.1 | 자동인쇄 1회 설정: 시작프로그램 + 크롬 바로가기 플래그 + 백그라운드 OFF. |
| v1.6.0 | 위치 저장 localStorage 단일화(재설치 없이 반영) + 저장 배너 + 자동인쇄 VBScript + 옵션 안내화. |
| **v1.7.0** | 편집기 개선: **마우스 휠 확대/축소** + **배경 클릭으로 안 닫힘**(편집 중 꺼짐 해결) + **브랜딩/회사명 텍스트 직접 입력**(editable) + 표시 토글 정상화. |
...
→ **`자동인쇄-설정.bat`(또는 `autoprint-setup.vbs`) 을 1회 더블클릭**하면, 이후엔
**평소처럼 크롬을 켜기만 해도** 인쇄창 없이 인쇄된다. (매번 실행 불필요)
> (v1.6.0: 한글 깨짐·실행정책 문제를 피하려고 VBScript 로 재구현. 더블클릭만으로 동작.)
스크립트가 하는 일:
