...
### 쓰기 계약 — write-free 계측으로 확보 (rev2.1에서 추가, 2026-07-20)

`form.submit`을 stub해 전송을 막고 페이로드만 캡처했다. 사후에 대상 레코드를 재조회해 **실제로 안 바뀐 것**까지 확인했다.

**`standby` — POST**

```
POST /jun/orderitem/orderItemStandby.do
  쿼리: tcode=order_item · status1=<목표상태> · status2=<요구 현재상태>
        · sKey=<페이지별 토큰> · reqPage · pageSize · 검색조건 24종
  본문: idx=<orderSeq>          ← 체크된 것만, 정확히 이 하나
```

- **본문에 orderSeq가 하나뿐임을 확인했다.** 숨은 배열·별도 필드 없음 → C의 전제조건 충족
- `status1`이 목표(`OS-`), `status2`가 요구 현재상태(`O--`). 서버가 현재상태를 검증한다는 뜻이므로 **우리 사전검증과 서버 검증이 이중으로 걸린다**
- **`sKey`는 페이지 로드마다 새로 발급되는 토큰이다.** 재사용 불가 — 갓 로드한 문서에서 얻어야 한다. `skin.js:717`이 다른 화면에서 같은 패턴을 이미 기록했다. iframe에 목록을 새로 띄우는 설계가 이 요구에 부합한다
- 검색조건은 **실제로 전달된다.** 저장소의 기존 미해결 항목 "standby 검색조건 미전달"은 다른 층위의 문제로 보인다(별건)

**`setCurrent` — GET 이동 (폼이 아니다)**

```
location = /jun/orderitem/orderItemPopCurrentSettingModify.do
             ?tcode=order_item&orderSeq=…&shop=…&client=…&orderDate=…
             &reqPage=…&pageSize=…&searchSortType=seq&…&barcode=<선택바코드>
```

- 970자. `opener`·`self.close`·`form.submit`·`action` 을 **쓰지 않는다.** `location` 대입뿐이다
- **⚠ GET이 쓰기를 수행한다.** 따라서 이 URL을 "확인용으로 한 번 받아보는" 행위 자체가 배정이다. **절대 조회 목적으로 fetch 하지 말 것**
- 반대로 설계에는 유리하다 — iframe에서 그냥 이동시키고 도착 페이지를 관찰하면 된다. 폼·POST·charset 문제가 없다

**팝업 iframe 적합성** — `X-Frame-Options` 없음, iframe 로드 정상, `document.characterSet = EUC-KR` (브라우저가 직접 확인)

**후보 목록은 살아있다** — 같은 주문의 후보가 30분 사이 144건 → 142건, 첫 바코드가 `2604O7` → `2604O9`로 바뀌었다(그 사이 매장 직원이 정상 처리). **"첫 행"은 실행 시점에 결정되는 값이지 고정값이 아니다.** 사전검증에서 보여준 바코드와 실제 배정 바코드가 다를 수 있으므로, 사용자에게 보여줄 때 이 점을 명시하고 성공 판정은 **실행 시점에 고른 바코드** 기준으로 한다.
...
>
> **⚠⚠ 차단은 `form.submit`만으로 부족하다 — 실제로 당했다(2026-07-20).**
> `standby`는 `form3.submit()`이라 폼 stub으로 잡혔지만, **`setCurrent`는 폼을 전혀 쓰지 않고 `location` 할당(GET 이동)이다.** `submit`/`requestSubmit`만 stub한 채 `setCurrent`를 호출했고, iframe을 즉시 제거한 덕에 대기 중이던 이동이 취소돼 우연히 쓰기가 나가지 않았다. 운이었다.
> → write-free 프로브는 **`location` 대입 · `location.assign/replace` · `window.open` · `form.submit` · `requestSubmit` · 앵커 클릭을 전부** 막아야 한다. 호출 후에는 대상 레코드를 재조회해 **실제로 안 바뀌었는지 확인**한다.

**진행 상황**: 5·6·9는 2026-07-20에 **완료**(1절 "쓰기 계약" 참조). 나머지는 확장 쪽이라 SHELL 배포 + 크롬 재시작이 필요하다.
