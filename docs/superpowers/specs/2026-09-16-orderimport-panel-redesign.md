...
## 5d. 고객 등록 보강 두 가지 (사장님 추가 요청 2026-09-16, SHELL 4.2.6)

**① 서버가 '휴대폰이 전화번호와 중복' 으로 등록을 거부하면 → 휴대폰을 비우고 번호는 비고에.**
라이브에서 `register_failed:휴대폰이 전화번호와 중복인 고객이 되었습니다.\n\n다시 입력하세요!` 로 건너뛴 카페24 건이 나왔다.
원인: 실행 전 휴대폰 검색(`searchWordType=phone`)은 **휴대폰(신부) 열만** 보므로, 다른 고객의 **전화번호(신랑)** 칸에 같은 번호가 있으면 못 거르고 서버가 거부한다.
사장님 규칙: "휴대폰(신부) 항목만 비워두고 전화번호는 비고란에 기입해서 등록".
- `registerClient(name, phone, clientJob, remark)` — `remark` 가 고객관리 폼의 **비 고** textarea(`name=remark`, maxlength 200, 실측)에 실린다.
- 실행기(`oiRunOrder` 고객 단계): 휴대폰을 비우는 **두 경우 모두** 번호를 비고에 남긴다 — (a) 검색에서 다른 고객이 같은 휴대폰을 쓸 때(기존 `new_nophone`), (b) 등록 POST 가 `oiPhoneDupMsg` (`/휴대폰[\s\S]*중복/`) 문구로 거부됐을 때. (b) 는 **재등록 전에 이름으로 다시 찾아** 이미 생겼으면 재사용(중복 고객 방지), 없으면 휴대폰 `''` + 비고 번호로 **한 번만** 더 POST. 휴대폰이 애초에 비어 있던 등록의 거부는 재시도하지 않는다. 그 밖의 거부 문구는 종전대로 `register_failed`.
- 진행 문구 `register_retry` = '고객 등록(휴대폰 비움 재시도)'.

**② 카페24 주문 중 판매금액 대비 정산금액 차이가 60% 이상이면 → 고객 등록의 마켓만 '지인소개'.**
"고객명 규칙은 /카 로 가되, 고객 등록 시 선택하는 마켓만 지인소개로."
- `oiReferralRatio(order)` = (Σ 판매가×수량 − Σ 정산금액) / Σ 판매가×수량 — **파일 값 그대로**(사은품 줄도 판매가가 적혀 있으면 그 값으로 합산, 파일의 합계 행과 같은 셈). 검토 표에서 편집한 판매가·수량은 판정에 넣지 않는다(파일에 판매가가 없으면 판정 불가 → 원래 마켓, 보수적). 어느 줄이든 판매가·정산금액이 없거나 판매금액이 0 이면 `null`(판정 불가). 주문장 단위(고객 등록이 주문장당 한 번).
- `oiClientJob(order)`: 마켓 코드가 카페24(`6`)이고 비율 ≥ 0.6 이면 `'19'`(지인소개), 아니면 원래 `clientJob`. 접미(`카`)·고객명은 그대로.
- UI: `toRunOrder` 가 `market.clientJob` 만 `oiClientJob(o)` 로 바꿔 싣고, 검토 표 고객 칩에 `· 마켓 지인소개(정산 차 60%↑)` 를 붙인다. 등록되는 곳은 `registerClient` 의 `clientJob` 하나뿐이라 이미 있는 고객(재사용)은 영향 없다.
- 수량 열이 있는 파일에서 정산금액은 **줄 합계**로 본다(현재 내보내기에는 수량 열이 없어 행=1개).
