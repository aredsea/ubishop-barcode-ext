...
## 5.1 v3.6.9 후속 (사용자 피드백 반영, 2026-07-19)

v3.6.8 라이브 확인 후 사용자가 두 가지를 알려왔다.

**(1) 필터에서 벗어난 행은 제거한다.** 배정으로 `입고완료` 가 되면 `본사확인` 필터 조건에서
빠지므로 목록에서 사라지는 게 맞다(초안은 무조건 제자리 갱신이었다).
- `statusMatchesLabel(상태셀텍스트, 상태필터 옵션라벨)` 로 판정 → 맞으면 갱신, 아니면 행 제거
  + `총 : N 개` 감소(안 줄이면 개수와 목록이 어긋난다).
- **상태 필터가 없으면 그 건은 목록에 계속 속하므로 갱신** 쪽이다.
- ⚠ 옵션 라벨의 괄호는 떼고 비교한다. 셀의 괄호는 바코드인데 옵션에도 괄호가 있는 항목이
  있어(`출고확인(매장재고)`) 그대로 비교하면 실제 셀 `출고확인(240HTI)` 과 안 맞아
  **멀쩡한 행을 지운다.** 판단이 안 서면 남기는 쪽으로 기운다(잘못 지우는 게 더 위험).

**(2) 선택취소(`cancelForm`)도 같은 문제라 같이 처리한다.**
- 취소 트리거 실측: `<input type="button" value="선택취소" onclick="cancelForm('2604O0')">`.
  결과 페이지는 `orderItemPopCurrentSettingCancel.do`.
- 가로채기에서 미배정 게이트를 없애 **배정·취소 둘 다** 가로챈다.
- 판정을 `changeConfirmed(관측, 기대, 직전)` 로 통일: `관측===기대 && 관측!==직전`.
  배정 `'' → 2604O0`, 취소 `2604O0 → ''` 가 대칭으로 풀린다. 직전 값은 가로챌 때
  `asgIssued(nonce → {orderSeq, beforeBarcode})` 에 기록한다.
- ⚠ **취소 결과 URL 의 `barcode` 는 '방금 뗀' 값**이라 기대값으로 쓰면 영원히 확인되지 않는다
  → `cancel` 이면 기대값을 `''` 로 고정.

**작업 정체성 = nonce (검수에서 두 번 데인 자리).** 작업(operation)과 신호(signal)를 구분하지
않으면 다음이 깨진다 —
- 중복 판정 키를 `orderSeq+목표바코드` 로 잡으면 **취소는 목표값이 항상 `''`** 이라 같은 주문의
  서로 다른 취소가 한 작업으로 뭉개진다.
- 대기열 우선순위를 `sig.ts` 로 잡으면 `ts` 가 `intent`/`done` **신호마다** 찍히므로, 늦게 온
  현재 작업의 `done` 이 그 사이 들어온 **다음 작업의 신호를 밀어내** 통째로 유실시킨다.
→ 클릭 1회 = nonce 1개. 완료 기억·대기열 판정 모두 nonce 로. `asgActive`(orderSeq → 검증 중
  nonce)로 **현재 작업의 후속 신호는 대기열에 넣지 않고 병합**한다. 목표값·직전값은 검증에만 쓴다.

## 6. 범위 밖

- 상단 일괄 버튼(`standby`)의 검색조건 미전달 결함 — 별개 이슈로 남긴다.
- 취소로 상태가 되돌아가 **현재 필터에 새로 맞게 된 건을 목록에 삽입**하는 것(행 추가는 안 한다).
