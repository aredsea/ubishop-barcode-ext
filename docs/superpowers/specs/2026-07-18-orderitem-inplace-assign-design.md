...
## 5.2 v3.7.0 — 검증에서 **재동기화**로 (§3.3 대체, 2026-07-19)

§3.3 은 "이 작업이 기대값을 냈는지" 검증하는 구조였다. 그 위에 작업별 완료기억·대기열·
기대값 대조를 쌓았는데, 공동검수에서 **같은 자리의 결함이 네 라운드 연속** 나왔다.

| 라운드 | 결함 |
|---|---|
| 1 | 완료키가 `orderSeq+목표바코드` → 취소는 목표값이 항상 `''` 이라 서로 다른 취소가 뭉개짐 |
| 2 | 대기열 우선순위가 `sig.ts` → ts 는 신호마다 찍혀, 늦게 온 done 이 '다음 작업' 신호를 밀어냄 |
| 3 | 큐를 도입하니 만료·중복 신호에 걸려 뒤가 고립됨 / 큐 상한이 성공한 작업을 버림 |
| 4 | 같은 클릭의 intent·done 이 각각 12초 폴링 → 700ms 간격 조회 폭주 |

전부 **"작업(operation)을 재현하려 한" 데서** 나왔다. 그래서 재현을 그만뒀다.

**바뀐 구조** — `verifyAndPatchRow` → `resyncRow`:
- 신호는 **"이 행이 낡았다"** 는 뜻으로만 쓴다. nonce 는 '내 탭이 발급한 신호인가' 확인에만.
- 루프: 행을 찾고 → **화면에 표시된 바코드**(`asgShownBarcode`, 상태셀 링크의 3번째 인자)와
  **서버 재조회 값**을 비교 → 다르면 서버 쪽으로 맞추고 종료. 같으면 700ms 뒤 재시도,
  12초 지나면 **조용히** 종료(= 화면이 이미 서버와 같다 = 고칠 게 없다).
- 겹침은 `asgAgain`(orderSeq → 최신 신호)으로 "끝나고 한 번 더". 큐가 필요 없다 —
  어차피 매번 서버의 **현재값**을 보므로 중간 상태를 재생할 이유가 없다.
- 같은 클릭이 두 번 돌지 않게 `asgApplied`(반영에 성공한 nonce)만 기억한다.
  성공하지 못했으면 넣지 않아 뒤따르는 `done` 에게 기회를 남긴다.
- 사라진 것: `asgPending`/큐 상한/`asgActive`/`asgDone`/`asgRecentlyDone`/`changeConfirmed`.

**교훈**: 같은 자리에서 지적이 반복되면 그건 개별 버그가 아니라 **설계가 틀렸다는 신호**다.
패치를 더 얹으면 복잡도만 오르고 새 구멍이 생긴다.

## 5.3 v3.7.0 — 매입처 정보창 / 전표 기본탭 (신규)

**(A) 매입처 정보 플로팅창.** 전표의 매입처·발주처명을 누르면 상세를 화면 위 창으로 띄운다.
- 원본 `view(seq)` 는 팝업이 아니라 **페이지 이동**이라 쓰지 않는다(§1 과 같은 이유).
- 이름은 링크가 아니라 텍스트라 seq 가 없다 → `factoryList.do` 에서 **이름→seq 맵**을 만들어
  `chrome.storage` 에 캐시(6h). 실측 323건. **매입처 목록은 전 계정 공유**(사용자 확인)라
  계정별 분기는 없다. 그래도 상세를 열 때 이름을 대조하고 계정 전환 시 캐시를 버린다.
- **동명 매입처는 링크하지 않는다** — 첫 seq 로 확정하면 두 번째 업체를 눌렀을 때 다른 업체
  정보가 뜬다. 조용히 틀린 값을 보여주는 쪽이 안 눌리는 것보다 나쁘다.
- 컬럼명이 페이지마다 다르다(`발주처명`/`매입처`/`매입처(매장명)`/`매입처입고일`) → 인덱스를
  박지 않고 `isFactoryHeader` 로 헤더에서 찾는다. `매입처상품코드` 는 제외.
  ⚠ **헤더는 첫 매칭 행으로 한 번만** 정한다. 매 행에서 재판정하면 상품명에 '매입처'가 든
  데이터 행을 헤더로 오인해 열이 어긋난다.
- 상세는 원본 표를 그대로 넣으면 고정폭 `input` 때문에 가로로 잘린다 → [라벨][값]만 뽑아
  자체 레이아웃으로 그린다(값은 이스케이프). 이름 대조 실패 시 **표시하지 않는다**(fail-closed).

**(B) 전표 기본탭.** 5개 전표의 '장'과 '내역'은 서로 다른 페이지다.
- 진입 후 리다이렉트하지 않고 **좌측 메뉴 링크의 href 를 재작성**한다(추가 로드·루프 없음).
  되돌릴 때를 위해 원본 href 를 `data-ub-tab-orig` 에 보관·복원.
- ⚠ **링크 텍스트가 메뉴 글자와 정확히 일치**해야 한다. 경로만 보면 같은 화면의
  **페이지네이션([2][3])이 같은 pathname·같은 tcode** 라 함께 바뀌어 페이징이 깨진다.
- 설정 UI 는 `isTabPrefPath` 로 그 전표 화면(장/내역 10경로)에서만 노출한다.
  단 **링크 재작성 자체는 전 화면에서** 돈다 — 메뉴는 어디서든 떠 있기 때문이다.
