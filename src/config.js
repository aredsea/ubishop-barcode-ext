...
(function () {
  ...
  window.UBCFG = {
    ...
    /* --- 메인석입고 페이지(inputStoneWriteForm.do) 매핑 (실측 확정 2026-08-05) --
     *  상품입고와 열 구성이 거의 같지만 **판매가 열이 다르다(11 → 10)**.
     *  상품입고의 순금·멜리입고가 열이 없고 대신 부가세·입고금액·감정서 열이 있다.
     *  실측 헤더: 0 No · 2 바코드/상품번호 · 3 구분 · 4 매장명(고객명)+상품정보 ·
     *            5 수량 · 6 공임 · 7 입고공급가 · 8 부가세 · 9 입고금액 ·
     *            10 판매가 · 11 감정서 · 12 연결번호 · 13 수정
     *  idx 체크박스 value 도 복합("순번,바코드,0")이라 상품입고와 같은 규칙이 통한다.
     *  ⚠ 메인석은 금속/중량 대신 캐럿 표기(예 "0.59 ct")라 metal·weight 가 비는 것이
     *    정상이다 — 라벨 문구는 프로그램 쪽 레이아웃이 담당한다.                 */
    inboundStone: {
      match: /input\/stone\/inputStone/i,
      cell: { info2: 2, gubun: 3, store: 4, price: 10 }
    },
