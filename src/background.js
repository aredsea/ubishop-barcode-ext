...
//  ★레지스트리 갱신은 직렬화한다 — get→수정→set 사이에 다른 호출이 끼어들면 마지막 set 이
//   앞선 기록을 덮어써(lost update) 창 하나가 레지스트리에서 사라진다. 그 창은 신원조회가
//   false 라 화면 정리도, 저장 후 갱신·자동 닫기도 영영 안 된다(수정 링크 연타에서 실제로 발생).
let _ubEpChain = Promise.resolve();
function ubEpSerialize(fn) {
  const next = _ubEpChain.then(fn, fn);
  _ubEpChain = next.then(() => {}, () => {});   // 실패해도 체인은 계속 흐르게
  return next;
}
//  ⚠ 읽기 실패를 빈 레지스트리로 바꾸지 않는다. 그러면 살아 있는 창을 '없다' 고 판단해 새
//   창을 또 열어 창이 둘로 늘어난다. 키가 없는 정상 첫 사용은 reject 가 아니라 빈 객체다.
//   실패는 위로 던져 ubEpOpen 이 확정 실패로 답하게 하고, 그러면 클릭이 네이티브로 복구된다.
async function ubEpAll() {
  const o = await chrome.storage.session.get(UB_EP_KEY);
  return (o && o[UB_EP_KEY]) || {};
}
//  ⚠ 실패를 삼키지 않는다. 삼키면 창은 떴는데 추적은 안 되는 미아 상태를 ok 로 보고하게 된다.
async function ubEpSet(map) {
  await chrome.storage.session.set({ [UB_EP_KEY]: map });
}
...
function ubEpDelete(winId) {
  return ubEpSerialize(async () => {
    const all = await ubEpAll();
...
async function ubEpOpen(msg, sender) {
  ...
  //  ★창 생성·기록 전체를 직렬화한다. 조회와 기록 사이에 다른 클릭이 끼어들면 두 창이
  //   같은 스냅샷 위에 각자 기록해 하나가 유실된다.
  return ubEpSerialize(async () => {
    ...
    for (const [winIdStr, rec] of Object.entries(all)) {
      ...
      //  ★기록을 **이동보다 먼저** 쓴다. 반대로 하면 새 문서가 빨리 뜨는 경우 그 문서의
      //   신원조회가 아직 옛 주문 레코드를 읽어, 새 주문 폼에 옛 주문키로 감시를 걸게 된다.
      //  ★createdAt 도 갱신한다 — 완료 처리가 '내가 읽은 그 레코드인가' 를 createdAt+orderSeq
      //   로 보는데, **같은 주문**을 다시 열면 orderSeq 가 그대로라 갱신 없이는 구분이 안 된다.
      const prev = { orderSeq: rec.orderSeq, createdAt: rec.createdAt };
      rec.orderSeq = orderSeq;
      rec.createdAt = Date.now();
      all[winIdStr] = rec;
      //  ⚠ 선기록이 실패하면 **이동하지 않는다**. 이동만 하면 화면은 B 인데 기록은 A 라,
      //   그 창이 자기를 A 로 알고 A 의 완료 처리에 닫힐 수 있다. 확정 실패로 답한다.
      try { await ubEpSet(all); }
      catch (e) {
        ubEpLog('재사용 기록 갱신 실패 → 이동하지 않는다', e && e.message);
        return { ok: false, error: 'registry_write_failed' };
      }
      //  ⚠ '창이 죽었다' 판정과 '저장소가 실패했다' 를 섞으면 안 된다. 섞으면 살아 있는 창의
      //   기록을 지우고 새 창을 또 열어 창이 둘로 늘어난다. 창 조작만 try 로 감싼다.
      let alive = true;
      try {
        await chrome.tabs.update(rec.tabId, { url: msg.url });
        await chrome.windows.update(winId, { focused: true });
      } catch (_) { alive = false; }    // 사용자가 이미 닫은 창
      if (alive) {
        ubEpLog('기존 창 재사용', winId, orderSeq);
        return { ok: true, windowId: winId, reused: true };
      }
      //  이동 실패 = 죽은 창. 방금 쓴 기록을 되돌리고(원상복구) 그 항목을 버린 뒤 새로 만든다.
      rec.orderSeq = prev.orderSeq;
      rec.createdAt = prev.createdAt;
      delete all[winIdStr];
      try { await ubEpSet(all); } catch (_) {}
    }
    ...
    //  ⚠ 창이 뜬 뒤에야 windowId 를 안다. 팝업 skin.js 는 DOMContentLoaded 에서 물어보므로
    //   (문서 파싱 뒤) 이 기록이 먼저 끝난다. 그래도 혹시 몰라 팝업 쪽에 1회 재시도가 있다.
    try {
      const fresh = await ubEpAll();
      fresh[String(win.id)] = { openerTabId: tabId, tabId: popTab.id, orderSeq, createdAt: Date.now() };
      await ubEpSet(fresh);
    } catch (e) {
      //  기록에 실패하면 그 창은 추적 불가(정리도 자동닫기도 안 됨) → 미아를 남기지 않고 닫는다.
      ubEpLog('레지스트리 기록 실패 → 방금 만든 창을 닫는다', e && e.message);
...
//  ★gen(=createdAt) 을 함께 준다. 팝업이 완료를 알릴 때 이걸 되돌려주면, 그 사이 창이 다른
//   주문으로 재사용됐는지 background 가 **처음 읽는 순간부터** 구분할 수 있다. 안 그러면
//   지연된 옛 완료 메시지가 지금 레코드를 자기 것으로 읽어 방금 연 창을 닫는다.
async function ubEpWhoAmI(sender) {
  ...
  return { isEditPopup: true, orderSeq: rec.orderSeq, gen: rec.createdAt };
}
...
async function ubEpDone(msg, sender) {
  ...
  //  ★팝업이 자기 신원(orderSeq+gen)을 실어 보낸다. 지연된 옛 완료 메시지가 그 사이 다른
  //   주문으로 재사용된 창을 자기 것으로 오인해 닫는 것을 **첫 읽기부터** 막는다.
  //  ⚠ 신원값이 **없으면 거부**한다(fail-closed). '있을 때만 대조' 로 두면 값을 안 실은
  //   메시지가 세대 보호를 통째로 우회한다 — 옛 호출부·회귀·잘못된 내부 메시지가 그 구멍이다.
  const claimSeq = String((msg && msg.orderSeq) == null ? '' : msg.orderSeq).trim();
  const claimGen = Number(msg && msg.gen);
  if (!claimSeq || !Number.isFinite(claimGen)) return { ok: false, error: 'identity_required' };
  if (claimSeq !== String(rec.orderSeq)) return { ok: false, error: 'stale_order' };
  if (claimGen !== Number(rec.createdAt)) return { ok: false, error: 'stale_gen' };
  //  ★목록 갱신을 **먼저** 확인하고, 성공했을 때만 창을 닫는다. 갱신도 못 했는데 창까지
  //   닫으면 사용자는 옛 값이 남은 목록만 보고 재시도할 방법도 없다.
  //  ★frameId:0 필수 — 안 주면 탭의 **모든 프레임**에 간다. 이 목록 페이지엔 ERP 자체
  //   iframe 이 10개 있고, 각 프레임의 수신부가 자기 form1.submit() 을 부르면 우리가 의도한
  //   적 없는 폼이 제출된다. 수신부에도 top+목록 가드가 있지만 여기서 먼저 좁힌다.
  let refreshed = false;
  try {
    ...
    if (!refreshed) ubEpLog('목록 갱신이 걸리지 않았다', res && res.error);
  } catch (e) { ubEpLog('목록 탭에 알리지 못함(닫혔을 수 있다)', e && e.message); }

  if (!refreshed) {
    //  창을 열어둔 채 실패를 알린다. 레지스트리도 남겨 사용자가 다시 시도할 수 있게 한다.
    return { ok: false, error: 'list_refresh_failed' };
  }
  //  ★목록 응답을 기다리는 사이 사용자가 다른 행의 [수정] 을 눌러 이 창이 **다른 주문으로
  //   재사용**됐을 수 있다(ubEpOpen 의 재사용 분기). 그때 이 완료 처리가 창을 닫으면 방금
  //   연 수정폼이 눈앞에서 사라진다. 삭제·닫기를 직렬화하고 레코드가 그대로인지 다시 본다.
  //  ⚠ 여기서 ubEpDelete 를 부르면 안 된다 — 그 함수도 직렬화라 자기 차례를 기다리다 교착된다.
  return ubEpSerialize(async () => {
    const now = await ubEpGet(winId);
    if (!now || now.createdAt !== rec.createdAt || now.orderSeq !== rec.orderSeq) {
      ubEpLog('처리 중 창이 다른 주문으로 재사용됐다 → 닫지 않는다', winId);
      return { ok: true, closed: false };
    }
    const all = await ubEpAll();
    delete all[String(winId)];
    try { await ubEpSet(all); } catch (e) { ubEpLog('레지스트리 정리 실패(무해)', e && e.message); }
...
chrome.windows.onRemoved.addListener((winId) => {
  ubEpDelete(winId)
    .then((had) => { if (had) ubEpLog('창 닫힘', winId); })
    .catch((e) => ubEpLog('창 닫힘 정리 실패(기록이 남을 수 있다)', e && e.message));
});
